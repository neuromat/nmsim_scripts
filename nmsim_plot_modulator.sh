#! /bin/bash
# Last edited on 2021-01-11 07:51:09 by jstolfi

# Reads the trace of a set of consecutive neurons in an elem_level simulation.
# Plots the {M} or {H} modulator as a function of time.
...
vLo="$1"; shift   # Min value to plot.
vHi="$1"; shift   # Max value to plot.
...
if [[ ${ineLo} -lt ${ineHi} ]]; then
  title="Neurons ${ineLo} .. ${ineHi} - ${varname} modulator ${which}[t] (mV)"
else
  title="Neuron ${ineLo} - ${varname} modulator ${which}[t] (mV)"
fi
...
gnuplot <<EOF
  ...
  plot \
    "< egrep -e '^[ ]*[0-9]' ${fname}" \
      using 1:${pltexpr} title "${which}[t]" with histeps lt 1 lw 2 lc rgb '#0022cc', \
